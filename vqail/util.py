...
def read_sweepconfig(
    env_id, algo, sweep_set, custom_hyperparams=None, verbose=0
) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    # Code taken from Stable Baselines 3 Zoo
    # Load hyperparameters from yaml file
    print(sweep_set)
    with open(f"sweep_configs/{env_id}_{algo}{sweep_set}.json", "r") as f:
        data = json.load(f)
        # hyperparams_dict = yaml.safe_load(f)
        # if env_id in list(hyperparams_dict.keys()):
        #     hyperparams = hyperparams_dict[env_id]
...
def save_model(model, ep_mean_rewards, log_dir, env_name, seed,run_id,cuda_id):
    log_path = os.path.join(log_dir, "outputs")
    Path(log_path).mkdir(exist_ok=True)
    
    print(f"Saving model at {log_path}")

    path = os.path.join(log_path, "{}-{}-{}".format(model.name, seed, env_name))
...
def save_model_checkpoint(model_name, model_state_dict, il_iteration, ppo_timesteps, log_dir, env_name, seed,run_id,cuda_id):
    log_path = os.path.join(log_dir, "checkpoints")
    # print("saving checkpoint to "+str(log_path))
    os.makedirs(log_path,exist_ok=True)
    print(f"Saving model checkpoints at {log_path}")

    path = os.path.join(log_path, "{}-{}-{}-{}-{}-{}-{}.pt".format(model_name, seed, env_name, cuda_id, run_id, il_iteration, ppo_timesteps))
